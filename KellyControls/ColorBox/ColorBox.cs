...
namespace KellyControls
{
	...
	/// </summary>
	[ToolboxItem(true)]
	[ToolboxBitmap(@"C:\Source\Repos\TheOneAndOnlyKelly\Utilities\KellyControls\ColorBox\ColorBox.bmp")]
	public partial class ColorBox : UserControl
	{
		...
		/// </summary>
		[Description("Draw style of the contol (Hue, Saturation, Brightness, Red, Green or Blue)")]
		...
		/// </summary>
		[Description("Shows the components of the currently selected color in its Hue-Saturation-Lightness breakdown")]
		public HSL HSL
		{
			get { return _hsl; }
			set
		...
		/// </summary>
		[Description("Shows the components of the currently selected color in its Red-Green-Blue breakdown")]
		public Color RGB
		{
			get { return _rgb; }
			set
		...
		private void DrawMarker(int x, int y, bool unconditional)
		{
			...
			Graphics.DrawEllipse(pen, x - 3, y - 3, 10, 10);
		...
		private void DrawBorder()
		{
			using (Graphics g = this.CreateGraphics())
			{
				...
				var Pen = new Pen(Color.FromArgb(172, 168, 153));    //	The same gray color used by Photoshop
				g.DrawLine(Pen, this.Width - 2, 0, 0, 0);    //	Draw top line
				g.DrawLine(Pen, 0, 0, 0, this.Height - 2);   //	Draw left hand line

				Pen = new Pen(Color.White);
				g.DrawLine(Pen, this.Width - 1, 0, this.Width - 1, this.Height - 1); //	Draw right hand line
				g.DrawLine(Pen, this.Width - 1, this.Height - 1, 0, this.Height - 1);    //	Draw bottome line

				Pen = new Pen(Color.Black);
				g.DrawRectangle(Pen, 1, 1, this.Width - 3, this.Height - 3); //	Draw inner black rectangle
			}
		}
		...
		private void Draw_Style_Hue()
		{
			...
			for (int i = 0; i < this.Height - 4; i++)
			{
				...
				hsl_start.L = 1.0 - (double)i / (this.Height - 4);
				hsl_end.L = hsl_start.L;

				LinearGradientBrush br = new LinearGradientBrush(new Rectangle(2, 2, this.Width - 4, 1),
		...
		private void Draw_Style_Saturation()
		{
			...
			for (int i = 0; i < this.Width - 4; i++)
			{
				...
				hsl_start.H = (double)i / (this.Width - 4);
				hsl_end.H = hsl_start.H;

				LinearGradientBrush br = new LinearGradientBrush(new Rectangle(2, 2, 1, this.Height - 4),
																 ColorManager.HSL_to_RGB(hsl_start),
																 ColorManager.HSL_to_RGB(hsl_end),
																 90, false);
		...
		private void Draw_Style_Red()
		{
			...
			for (var i = 0; i < this.Height - 4; i++)
			{
				...
				var br = new LinearGradientBrush(new Rectangle(2, 2, this.Width - 4, 1),
		...
		private void Draw_Style_Green()
		{
			...
			for (var i = 0; i < this.Height - 4; i++)
			{
				...
				var red = Round(255 - (255 * (double)i / (this.Height - 4)));
		...
		private void Draw_Style_Blue()
		{
			...
			{
				//	Calculate Green at this line (Red and Blue are constant)
				var green = Round(255 - (255 * (double)i / (this.Height - 4)));
		...
		private void Reset_Marker(bool redraw)
		{
			...
			{
				case eDrawStyle.Hue:
					_markerX = Round((this.Width - 4) * _hsl.S);
					_markerY = Round((this.Height - 4) * (1.0 - _hsl.L));
					break;
				case eDrawStyle.Saturation:
					_markerX = Round((this.Width - 4) * _hsl.H);
					_markerY = Round((this.Height - 4) * (1.0 - _hsl.L));
					break;
				case eDrawStyle.Brightness:
					_markerX = Round((this.Width - 4) * _hsl.H);
					_markerY = Round((this.Height - 4) * (1.0 - _hsl.S));
					break;
				case eDrawStyle.Red:
					_markerX = Round((this.Width - 4) * (double)_rgb.B / 255);
					_markerY = Round((this.Height - 4) * (1.0 - (double)_rgb.G / 255));
					break;
				case eDrawStyle.Green:
					_markerX = Round((this.Width - 4) * (double)_rgb.B / 255);
					_markerY = Round((this.Height - 4) * (1.0 - (double)_rgb.R / 255));
					break;
				case eDrawStyle.Blue:
					_markerX = Round((this.Width - 4) * (double)_rgb.R / 255);
					_markerY = Round((this.Height - 4) * (1.0 - (double)_rgb.G / 255));
					break;
			}
		...
		private HSL GetColor(int x, int y)
		{
			...
			switch (_drawStyle)
			{
				...
				case eDrawStyle.Brightness:
					...
					break;
				case eDrawStyle.Red:
					_hsl = ColorManager.RGB_to_HSL(Color.FromArgb(_rgb.R, Round(255 * (1.0 - (double)y / (this.Height - 4))), Round(255 * (double)x / (this.Width - 4))));
					break;
				case eDrawStyle.Green:
					_hsl = ColorManager.RGB_to_HSL(Color.FromArgb(Round(255 * (1.0 - (double)y / (this.Height - 4))), _rgb.G, Round(255 * (double)x / (this.Width - 4))));
					break;
				case eDrawStyle.Blue:
					_hsl = ColorManager.RGB_to_HSL(Color.FromArgb(Round(255 * (double)x / (this.Width - 4)), Round(255 * (1.0 - (double)y / (this.Height - 4))), _rgb.B));
					break;
			}

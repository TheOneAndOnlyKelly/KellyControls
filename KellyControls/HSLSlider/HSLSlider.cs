...
namespace KellyControls
{
	...
	/// </summary>
	[ToolboxItem(true)]
	[ToolboxBitmap(@"C:\Source\Repos\TheOneAndOnlyKelly\Utilities\KellyControls\HSLSlider\HSLSlider.bmp")]
	public partial class HSLSlider : UserControl
	{
		...
		private bool _fillMarker = false;
		private bool _reverseFill = false;
		private bool _snapToIndicator = false;
		...
		{
			get { return (_orientation == System.Windows.Forms.Orientation.Vertical); }
		}
		...
		/// </summary>
		[DefaultValue(typeof(eDrawStyle), "Hue"), Description("The drawstyle of the contol (Hue, Saturation, Luminance).")]
		...
		/// </summary>
		[DefaultValue(typeof(Orientation), "Horizontal"), Description("Orientation of the control.")]
		public Orientation Orientation
		{
			...
			set
			{
				...
				if (((_orientation == Orientation.Horizontal) && (this.Height > this.Width)) ||
					((_orientation == Orientation.Vertical) && (this.Width > this.Height)))
				{
		...
		/// </summary>
		[DefaultValue(typeof(bool), "true"), Description("Indicates whether a border line should be drawn about the slider bar itself.")]
		...
		/// </summary>
		[DefaultValue(typeof(bool), "false"), Description("Indicates whether the marker should be filled with the value indicated.")]
		...
		[DefaultValue(typeof(double), "1"), Description("Transparency of the color.")]
		...
		/// </summary>
		[DefaultValue(typeof(int), "6"), Description("Size, in pixels, of the marker.")]
		...
		/// </summary>
		[DefaultValue(typeof(Color), "ControlDarkDark"), Description("Color to draw the marker.")]
		...
		/// </summary>
		[DefaultValue(typeof(eMarkerLayout), "BottomOrRight"), Description("Gets or sets the style of marker, left/top, bottom/right, or both.")]
		...
		/// </summary>
		[DefaultValue(typeof(eMarkerStyle), "Triangle"), Description("Gets or Sets the style of marker that should be used.")]
		...
		/// </summary>
		[DefaultValue(typeof(double), "100"), Description("Maximum value for the control.")]
		...
		/// </summary>
		[DefaultValue(typeof(double), "0"), Description("Minimum value for the control.")]
		...
		/// </summary>
		[DefaultValue(typeof(double), "50"), Description("Value the control is currently set at.")]
		public double Value
		{
			get
			{
		...
		[DefaultValue(typeof(bool), "False"), Description("Reverse the direction the color fill is drawn at.")]
		...
		/// </summary>
		[DefaultValue(typeof(Color), "Black"), Description("Color to draw the indicator Marks..")]
		...
		[DefaultValue(typeof(bool), "False"), Description("Causes the marker to snap to the indicator when nearby.")]
		...
		private void DrawBar(Graphics g, Rectangle drawArea, HSL startColor, int sgn, bool allowWrap)
		{
			var HSL = new HSL(startColor);
			...
			using (var LinePen = new Pen(Color.White))
			{
				...
				{
					var Position = CalcPositionFromValue(_indicatorMarks[i]);
					if (IsVertical)
					{
						Y1 = Position.Y;
						Y2 = Position.Y;
		...
		{
			if (offset)
				DrawBar(g, drawArea, new HSL(1 - this.ValuePercent(_value), 1, 1), +1, true);
			else
				DrawBar(g, drawArea, new HSL(0.5, 1, 1), +1, true);
		}
		...
		{
			if (_reverseFill)
				DrawBar(g, drawArea, new HSL(0, 0, 0), -1, false);
			else
				DrawBar(g, drawArea, new HSL(0, 0, 1), -1, false);
		}
		...
		private void DrawBar_Saturation(Graphics g, Rectangle drawArea, bool reverseFill, bool inverse)
		{
			...
			if (reverseFill)
				StartColor = new HSL(Hue, 0, 0.5);
			else
				StartColor = new HSL(Hue, 1, 1);
		...
		private void DrawBar_Alpha(Graphics g, Rectangle drawArea)
		{
			...
			if (_reverseFill)
				DrawBar(g, drawArea, new HSL(_hsl.H, 1, 1, 0), -1, false);
			else
				DrawBar(g, drawArea, new HSL(_hsl.H, 1, 1, 1), -1, false);
		}
		...
		private void DrawBarBorder(Graphics g, Rectangle drawArea)
		{
			...
			using (Pen LightPen = new Pen(SystemColors.ControlLight))
			{
		...
		/// <param name="topLeft">Indicates if this is the top marker for horizontal orientation, or the left marker for vertical orientation.</param>
		private void DrawMarker(Graphics g, Rectangle drawArea, bool topLeft)
		{
			...
			if (Path != null)
			{
				...
				using (Pen MarkPen = new Pen(_markerColor, PenWidth))
		...
		/// </summary>
		/// <param name="position">position value of the slider, lowest being at the bottom.  The range is between 0 and the controls height-9.  The values will be adjusted if too large/small</param>
		/// <param name="unconditional">If Unconditional is true, the slider is drawn, otherwise some logic is performed to determine is drawing is really neccessary.</param>
		private void DrawSlider(Graphics g)
		{
			if (!ShowMarker)
				return;

			Point MarkerPoint = CalcPositionFromValue(this.Value);

			int HalfSize = _markerSize / 2;
			Rectangle DrawArea = new Rectangle(0, 0, HalfSize * 2, HalfSize * 2);
			...
			if (IsVertical)
			{
				...
				{
					DrawArea = new Rectangle(0, 0, this.ClientRectangle.Width - this.Padding.Horizontal - 1, _markerSize)
					{
						Location = new Point(_startOffset, MarkerPoint.Y - HalfSize)
					};
					DrawMarker(g, DrawArea, true);
				}
			}
			else
			{
				...
				{
					DrawArea = new Rectangle(0, 0, _markerSize, this.ClientRectangle.Height - this.Padding.Vertical - 1)
					{
						Location = new Point(MarkerPoint.X - HalfSize, _startOffset)
					};
					DrawMarker(g, DrawArea, true);
				}
			}
		...
		private GraphicsPath GetDiamondPath(Rectangle drawArea)
		{
			...
			var Points = new List<PointF>
			{
				new PointF(drawArea.Left + HalfSize, drawArea.Top),
				new PointF(drawArea.Right, drawArea.Top + HalfSize),
				new PointF(drawArea.Left + HalfSize, drawArea.Bottom),
				new PointF(drawArea.Left, drawArea.Top + HalfSize)
			};
		...
		private void CalculateSizes()
		{
			if (IsVertical)
			{
				...
				_drawingWidth = this.ClientRectangle.Height - this.Padding.Vertical;
				_startOffset = this.Padding.Top;
				if (_barBorder)
				{
					_drawingWidth -= 2;
					_startOffset += 1;
				...
				if (ShowMarker && ((_markerLayout == eMarkerLayout.TopOrLeft) || (_markerLayout == eMarkerLayout.Both)))
					_z2 += _markerSize;
				_z4 = this.ClientRectangle.Width - this.Padding.Right;
				_z3 = _z4 - 1;
			...
			{
				_drawingWidth = this.ClientRectangle.Width - this.Padding.Horizontal;
				_startOffset = this.Padding.Left;
				if (_barBorder)
				{
					_drawingWidth -= 2;
					_startOffset += 1;
				...
				if (ShowMarker && ((_markerLayout == eMarkerLayout.TopOrLeft) || (_markerLayout == eMarkerLayout.Both)))
					_z2 += _markerSize;
				_z4 = this.ClientRectangle.Height - this.Padding.Bottom;
				_z3 = _z4 - 1;
				if (ShowMarker && ((_markerLayout == eMarkerLayout.BottomOrRight) || (_markerLayout == eMarkerLayout.Both)))
					_z3 -= _markerSize;
			}
		...
		private double CalcValueFromPosition(Point pt)
		{
			...
			if (_orientation == Orientation.Horizontal)
			{
				var X = pt.X;
				X -= _startOffset;
				Pct = (double)X / (double)_drawingWidth;
			}
			else
			{
				var Y = pt.Y;
				Y -= _startOffset;
				Pct = (double)Y / (double)_drawingWidth;
			}
			Pct = AdjustValue(Pct, false);
			var TotalValue = _maxValue - _minValue;

			var Value = (TotalValue * Pct) + _minValue;
			...
			if (_snapToIndicator)
			{
				...
				for (int i = 0; i < _indicatorMarks.Count; i++)
				{
					var IndPct = ValuePercent(_indicatorMarks[i]);
		...
		/// <summary>
		/// Determine the position of the control that corresponds to this value. If oriented vertically, returns the point along the left side
		...
		private Point CalcPositionFromValue(double value)
		{
			...
			{
				var y = (int)(_drawingWidth * ValuePercent(value)) + _startOffset;
				if (y <= _startOffset)
					y = _startOffset + 1;
				return new Point(this.Padding.Left + (_barBorder ? 1 : 0), y);
			}
			else
			{
				var x = (int)(_drawingWidth * ValuePercent(value)) + _startOffset;
				if (x <= _startOffset)
					x = _startOffset + 1;
				return new Point(x, this.Padding.Top + (_barBorder ? 1 : 0));
			}
		}
		...
		{
			var TotalValue = _maxValue - _minValue;
			var RelValue = value - _minValue;
			return (TotalValue != 0) ? RelValue / TotalValue : 0;
		}
		...
		/// <summary>
		/// Determines what color the markers should be filled with if we are filling them, based on what style we are drawing in.
		/// </summary>
		private Color GetMarkerFillColor()
		{
			...
			{
				case eDrawStyle.Hue:
					CalcValue = (float)this.Value / 360f;
					CalcValue = CalcValue > 1 ? 1 : CalcValue < 0 ? 0 : CalcValue;
					return ColorManager.HSL_to_RGB(new HSL(CalcValue));

				case eDrawStyle.Luminance:
					CalcValue = 1 - (float)(this.Value / 100.0);
					CalcValue = CalcValue > 1 ? 1 : CalcValue < 0 ? 0 : CalcValue;
					return ColorManager.HSL_to_RGB(new HSL(_hsl.H, 0, CalcValue));

				case eDrawStyle.Saturation:
					CalcValue = 1 - (float)(this.Value / 100.0);
					CalcValue = CalcValue > 1 ? 1 : CalcValue < 0 ? 0 : CalcValue;
					return ColorManager.HSL_to_RGB(new HSL(_hsl.H, CalcValue, 0.5 + CalcValue / 2));
				...
				case eDrawStyle.SaturationWithInversion:
					...
					{
						CalcValue = -CalcValue;
						return ColorManager.HSL_to_RGB(new HSL(AdjustValue(_hsl.H + 0.5, true), CalcValue, 0.5 + CalcValue / 2));
					}
					else
						return ColorManager.HSL_to_RGB(new HSL(_hsl.H, CalcValue, 0.5 + CalcValue / 2));
		...
		/// <summary>
		/// Determines which direction the marker is pointed at, based on the flag passed in, and the current orientation of the control
		/// </summary>
		/// <param name="topLeft">Indicates if the marker is the top marker for horizontal orientation, or the left marker for vertical.</param>
		private MarkerCardinal GetMarkerCardinal(bool topLeft)
		{
			if (IsVertical && topLeft)
				return MarkerCardinal.Left;
			else if (!IsVertical && topLeft)
		...
		{
			this.Value = CalcValueFromPosition(e.Location);
			this.Refresh();
		}
		...
		private void PaintControl(Graphics g)
		{
			...
			if (IsVertical)
			{
				Size = this.ClientRectangle.Height - this.Padding.Vertical;
				if (_barBorder)
					Size -= 2;
			...
			{
				Size = this.ClientRectangle.Width - this.Padding.Horizontal;
				if (_barBorder)
					Size -= 2;
			...
			switch (_drawStyle)
			{
				...
				case eDrawStyle.HueOffset:
					DrawBar_Hue(g, DrawArea, false);
					Rectangle HalfBar = new Rectangle(DrawArea.Location, DrawArea.Size);
					if (IsVertical)
					{
						HalfBar.Width /= 2;

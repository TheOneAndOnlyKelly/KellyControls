...
{
	[ToolboxItem(true)]
	[ToolboxBitmap(@"C:\Source\Repos\TheOneAndOnlyKelly\Utilities\KellyControls\ColorGrid\ColorGrid.bmp")]
	public partial class ColorGrid : UserControl
	{
		...
		private Image _cancelButtonImage = null;
		private Color _color = Color.Red;

		private Palette _baseList = new Palette();
		private Palette _customList = new Palette();
		private Palette _combinedList = new Palette();

		private int _customStartIndex = -1;
		private bool _embiggenHotTracked = false;
		...
		private Image _okButtonImage = null;
		private ColorPalette _colorPalette = ColorPalette.Default;
		private Control _parentUserControl = null;
		private int _selectedIndex = -1;
		private bool _showNames = false;
		private bool _showRGBOnToolTip = false;
		private bool _stretchToFitParent = false;
		private bool _suspendNativeAddButtonClick = false;
		...
		/// </summary>
		[DefaultValue(typeof(bool), "True"), Description("Indicates whether the \"Add Colors\" button should shown.")]
		...
		/// </summary>
		[DefaultValue(typeof(bool), "True"), Description("Indicates whether the \"No Color\" button should be shown in the dropdown.")]
		...
		[DefaultValue(typeof(Size), "16, 16"), Description("Size of the individual color blocks.")]
		...
		[DefaultValue(typeof(ColorPalette), "Default"), Description("Color Palette to display.")]
		...
		[DefaultValue(typeof(bool), "False"), Description("Shows the colors as individual lines, with the name adjacent.")]
		...
		[DefaultValue(typeof(bool), "False"), Description("Shows the RGB values of the color in the ToolTip.")]
		...
		[DefaultValue(typeof(bool), "False"), Description("Expand the height of this control to fit its parent control if shorter.")]
		...
		public ColorGrid()
		{
			...
			if (ColorExists(_color, false))
				this.SelectedIndex = IndexOf(_color, true);
			else
		...
		/// <returns>Returns true if the color is present in the color list. If inCustomColor is true, it will only check the custom list, if false, checks entire list. If the color is not prsent, returns false.</returns>
		...
		private List<string> GetSystemColorNames()
		{
			...
			foreach (System.Reflection.MemberInfo member in (typeof(System.Drawing.SystemColors)).GetProperties())
			{
		...
		private int IndexOf(Point point)
		{
			...
			for (int i = 0; i < _combinedList.Count; i++)
			{
				...
				{
					if ((point.X >= Loc.X) && (point.X < Loc.X + _gridSize.Width) &&
						(point.Y >= Loc.Y) && (point.Y < Loc.Y + _gridSize.Height))
						return i;
				}
			}
			return -1;
		...
		private int IndexOf(Color color, bool ignoreAlpha)
		{
			...
			for (int i = 0; i < _combinedList.Count; i++)
			{
				if ((ignoreAlpha ? true : _combinedList[i].Color.A == color.A) &&
					_combinedList[i].Color.R == color.R &&
					_combinedList[i].Color.G == color.G &&
					_combinedList[i].Color.B == color.B)
					return i;
			}
			return -1;
		}
		...
		protected void OnAddButtonClick()
		{
			if (_suspendNativeAddButtonClick)
				AddButtonClick?.Invoke(null, null);
		}
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			try
			{
				...
				if ((index != -1) && (_hoverIndex != index)) // Only update the tooltip and hotTracking if the actual index changes.
				{
					...
					toolTip1.Show(Text, this, new Point(e.Location.X, e.Location.Y + this.Cursor.Size.Height / 2));
					if (_hotTracking)
						Refresh();
				}
				else if (index == -1)
		...
		private void cmdAddColor_Click(object sender, EventArgs e)
		{
			...
			if (AddColorDialog.ShowDialog() != DialogResult.Cancel)
			{
				...
				else
				{
					...
					this.SelectedIndex = IndexOf(AddColorDialog.Color, true);
					if (Parent != null)
